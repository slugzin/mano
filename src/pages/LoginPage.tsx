...
const LoginPage: React.FC = () => {
  ...
  // Função para formatar WhatsApp - agora apenas remove caracteres não numéricos
  const formatWhatsApp = (value: string) => {
    // Remove apenas caracteres não numéricos, mantendo o input como digitado
  ...
  const getStepDescription = (): string => {
    switch (currentStep) {
      ...
      case CadastroStep.WHATSAPP:
        return 'Digite apenas os números do seu WhatsApp';
      case CadastroStep.EMAIL:
  ...
  const toggleAuthMode = () => {
    // Se estiver no login, vai para cadastro (isLogin = false)
    // Se estiver no cadastro, vai para login (isLogin = true)
    ...
    // Se for para cadastro, reseta para primeira etapa
    if (!newMode) {
      setCurrentStep(CadastroStep.NOME);
    }
  };
