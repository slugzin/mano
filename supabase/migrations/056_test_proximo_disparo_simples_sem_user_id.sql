-- Função de teste para proximo_disparo_simples sem user_id
CREATE OR REPLACE FUNCTION test_proximo_disparo_simples_sem_user_id()
RETURNS TABLE (
    debug_info JSON,
...
COMMENT ON FUNCTION test_proximo_disparo_simples_sem_user_id() IS 'Função para testar proximo_disparo_simples sem filtro de user_id.';

-- Log das alterações
DO $$
BEGIN
  RAISE NOTICE 'Função de teste criada para proximo_disparo_simples sem user_id';
  RAISE NOTICE 'Use: SELECT * FROM test_proximo_disparo_simples_sem_user_id()';
END $$; 