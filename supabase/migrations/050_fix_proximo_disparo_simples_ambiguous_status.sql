-- Corrigir ambiguidade da coluna status na função proximo_disparo_simples
...
COMMENT ON FUNCTION proximo_disparo_simples(UUID) IS 'Função RPC simplificada para n8n que aceita user_id como parâmetro. Se não fornecido, usa user_id padrão. Corrigida ambiguidade da coluna status.';

-- Log das alterações
DO $$
BEGIN
  RAISE NOTICE 'Função proximo_disparo_simples corrigida - ambiguidade da coluna status resolvida';
END $$; 