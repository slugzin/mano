...
RETURNS TABLE (
  ...
  tipo_midia TEXT,
  midia_url TEXT,
  status_disparo TEXT,  -- Mudado de status para status_disparo
...
BEGIN
  ...
  WHERE public.disparos_agendados.status = 'pendente'  -- Especificar tabela
  ...
  WHERE t.status = 'pendente'  -- Usar alias da tabela
  ...
  IF v_next_task.id IS NOT NULL THEN
    ...
    SELECT 
      ...
      'processando'::TEXT as status_disparo,  -- Especificar alias
