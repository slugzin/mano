-- Corrigir função agendar_disparos para incluir user_id
-- Remover função antiga se existir
DROP FUNCTION IF EXISTS agendar_disparos(INTEGER[], TEXT, TEXT, TEXT, TEXT, INTEGER, TEXT, INTEGER, TEXT, TEXT);

-- Criar função agendar_disparos com user_id
...
BEGIN
    ...
    FOR v_empresa IN 
        ...
        AND user_id = v_user_id -- Filtrar apenas empresas do usuário
...
COMMENT ON FUNCTION agendar_disparos IS 'Função para agendar disparos com suporte a user_id';

-- Log das alterações
DO $$
BEGIN
  RAISE NOTICE 'Função agendar_disparos corrigida com suporte a user_id';
  RAISE NOTICE 'Agora a função filtra empresas por user_id e inclui user_id na campanha';
END $$; 