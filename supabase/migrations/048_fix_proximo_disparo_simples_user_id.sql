-- Corrigir função proximo_disparo_simples para aceitar user_id como parâmetro
-- Remover função antiga
DROP FUNCTION IF EXISTS proximo_disparo_simples();

-- Criar nova função que aceita user_id como parâmetro
...
BEGIN
  ...
  ELSE
    ...
    ELSE
      ...
      -- Por enquanto, usar o mesmo user_id padrão
      v_user_id := 'c4a948a1-a1cc-42ed-902c-a70c4959d3b7'::UUID;
...
COMMENT ON FUNCTION proximo_disparo_simples(UUID) IS 'Função RPC simplificada para n8n que aceita user_id como parâmetro. Se não fornecido, usa user_id padrão.';

-- Função para testar a função simplificada com user_id específico
CREATE OR REPLACE FUNCTION test_proximo_disparo_simples(p_user_id UUID DEFAULT NULL)
RETURNS TABLE (
    resultado TEXT,
...
DECLARE
    ...
    v_user_id_used UUID;
BEGIN
    -- Chamar a função proximo_disparo_simples com user_id
    SELECT * INTO v_disparo FROM proximo_disparo_simples(p_user_id);
    
    -- Pegar o user_id usado
    v_user_id_used := COALESCE(p_user_id, 'c4a948a1-a1cc-42ed-902c-a70c4959d3b7'::UUID);
...
-- Comentário explicativo da função
COMMENT ON FUNCTION test_proximo_disparo_simples(UUID) IS 'Função para testar a função proximo_disparo_simples com user_id específico.';

-- Função para criar disparo de teste para um usuário específico
CREATE OR REPLACE FUNCTION criar_disparo_teste_usuario(p_user_id UUID)
RETURNS TEXT
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    INSERT INTO public.disparos_agendados (
        ...
        'Mensagem de teste para usuário específico',
        'nenhum',
        NULL,
        'pendente',
        NOW()::TIMESTAMP,
    ...
    RETURN 'Disparo de teste criado com ID: ' || v_id || ' para user_id: ' || p_user_id;
EXCEPTION
    WHEN OTHERS THEN
...
-- Comentário explicativo da função
COMMENT ON FUNCTION criar_disparo_teste_usuario(UUID) IS 'Função para criar disparo de teste para um usuário específico.';

-- Log das alterações
DO $$
BEGIN
  RAISE NOTICE 'Função proximo_disparo_simples atualizada para aceitar user_id como parâmetro';
  RAISE NOTICE 'Agora é possível chamar: SELECT * FROM proximo_disparo_simples(user_id_especifico)';
  RAISE NOTICE 'Ou ainda usar sem parâmetro para usar o user_id padrão do n8n';
END $$; 