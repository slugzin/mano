-- Corrigir ambiguidade da coluna id na função proximo_disparo_simples
...
BEGIN
  ...
  IF v_next_task.id IS NOT NULL THEN
    ...
    WHERE public.disparos_agendados.id = v_next_task.id;
...
COMMENT ON FUNCTION proximo_disparo_simples() IS 'Função RPC simplificada para n8n que busca qualquer disparo pendente sem filtro de user_id. Corrigida ambiguidade da coluna id.';

-- Log das alterações
DO $$
BEGIN
  RAISE NOTICE 'Função proximo_disparo_simples corrigida - ambiguidade da coluna id resolvida';
END $$; 