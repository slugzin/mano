-- Função RPC corrigida para processar variáveis da mensagem
...
DECLARE
    ...
    -- Variáveis para processamento da mensagem
    v_mensagem_processada TEXT;
    v_empresa_nome TEXT;
    v_empresa_categoria TEXT;
    v_empresa_cidade TEXT;
    v_empresa_endereco TEXT;
    v_empresa_website TEXT;
    v_empresa_avaliacao TEXT;
    v_empresa_posicao TEXT;
BEGIN
    v_now := NOW();
    ...
    LOOP
        BEGIN
            ...
            -- Substituir variáveis de cidade (extrair do endereço)
            IF v_empresa.endereco IS NOT NULL THEN
                v_empresa_cidade := split_part(split_part(v_empresa.endereco, ',', -2), '-', 1);
                v_empresa_cidade := trim(v_empresa_cidade);
                IF v_empresa_cidade != '' THEN
                    v_mensagem_processada := replace(v_mensagem_processada, '{cidade}', v_empresa_cidade);
                    v_mensagem_processada := replace(v_mensagem_processada, '{empresa_cidade}', v_empresa_cidade);
                END IF;
            END IF;
            
            -- Substituir variáveis de endereço
            IF v_empresa.endereco IS NOT NULL THEN
                v_mensagem_processada := replace(v_mensagem_processada, '{endereco}', v_empresa.endereco);
                v_mensagem_processada := replace(v_mensagem_processada, '{empresa_endereco}', v_empresa.endereco);
            END IF;
            
            -- Substituir variáveis de website
            IF v_empresa.website IS NOT NULL THEN
                v_mensagem_processada := replace(v_mensagem_processada, '{website}', v_empresa.website);
                v_mensagem_processada := replace(v_mensagem_processada, '{empresa_website}', v_empresa.website);
            END IF;
            
            -- Substituir variáveis de avaliação
            IF v_empresa.avaliacao IS NOT NULL THEN
                v_empresa_avaliacao := v_empresa.avaliacao::TEXT;
                v_mensagem_processada := replace(v_mensagem_processada, '{avaliacao}', v_empresa_avaliacao);
                v_mensagem_processada := replace(v_mensagem_processada, '{empresa_avaliacao}', v_empresa_avaliacao);
            END IF;
            
            -- Substituir variáveis de posição
            IF v_empresa.posicao IS NOT NULL THEN
                v_empresa_posicao := v_empresa.posicao::TEXT;
                v_mensagem_processada := replace(v_mensagem_processada, '{posicao}', v_empresa_posicao);
                v_mensagem_processada := replace(v_mensagem_processada, '{empresa_posicao}', v_empresa_posicao);
            END IF;
            
            -- Substituir variáveis de telefone
            IF v_empresa.telefone IS NOT NULL THEN
                v_mensagem_processada := replace(v_mensagem_processada, '{telefone}', v_empresa.telefone);
                v_mensagem_processada := replace(v_mensagem_processada, '{empresa_telefone}', v_empresa.telefone);
            END IF;

            -- INSERIR DISPARO AGENDADO COM MENSAGEM PROCESSADA
            INSERT INTO disparos_agendados (
                ...
                v_empresa.website,
                v_empresa.endereco,
                v_mensagem_processada, -- MENSAGEM COM VARIÁVEIS PROCESSADAS
                p_tipo_midia,
                p_midia_url,
                'pendente',
                v_agendado,
